...
describe("createEjentumTools factory", () => {
  it("returns eight tools keyed by camelCase developer names", () => {
    const tools = createEjentumTools();
    expect(Object.keys(tools).sort()).toEqual([
      "adaptiveAntiDeception",
      "adaptiveCode",
      "adaptiveMemory",
      "adaptiveReasoning",
      "antiDeception",
  ...
  it("tool ids match the canonical hyphenated mode strings", () => {
    const tools = createEjentumTools();
    expect(tools.reasoning.id).toBe("reasoning");
    expect(tools.code.id).toBe("code");
    expect(tools.antiDeception.id).toBe("anti-deception");
    expect(tools.memory.id).toBe("memory");
    expect(tools.adaptiveReasoning.id).toBe("adaptive-reasoning");
    expect(tools.adaptiveCode.id).toBe("adaptive-code");
    expect(tools.adaptiveAntiDeception.id).toBe("adaptive-anti-deception");
    expect(tools.adaptiveMemory.id).toBe("adaptive-memory");
  });
...
describe("per-tool factories", () => {
  ...
  it("createAdaptiveCodeTool mentions code and adaptive", () => {
  ...
  it("createAdaptiveAntiDeceptionTool mentions integrity and adaptive", () => {
    const t = createAdaptiveAntiDeceptionTool();
    expect(t.description.toLowerCase()).toMatch(/adaptive|rewrit/);
    expect(t.description.toLowerCase()).toMatch(/anti-deception|integrity|honest/);
  });
  it("createAdaptiveMemoryTool mentions memory and adaptive", () => {
